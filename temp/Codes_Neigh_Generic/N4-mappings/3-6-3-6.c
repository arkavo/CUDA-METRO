  ...
  //  printf("First neighbors (N4) of element at index %d:\n", index);
    ...
    for ( i = 0; i < 6; i++) {
        	 ...
        	 else if(i==4)
        {
			...
			if(row<(N/2)-2)
        	{
          	neighborRow=neighborRow+(N/2)+2;
	      	neighborCol=neighborCol+1;
			}			
			else if(row==((N/2)-2)||row==((N/2)-1))
			{
          	neighborRow=neighborRow+2;
	      	neighborCol=neighborCol+1;
			}
...
int main() {
    ...
    int matrix[N * N];
      FILE *fptr;
   fptr= fopen("C:\\Users\\deric\\OneDrive - Indian Institute of Science\\Desktop\\Major_Project\\Codes_Neigh_Generic\\N4-mappings\\output3.txt","w"); 
         if(fptr == NULL)
   {
      printf("Error!");   
      exit(1);             
