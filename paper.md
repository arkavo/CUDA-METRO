...
Fig 2: Presence of skyrmions in $\textrm{MnBr_2}$ and $\textrm{CrInSe_3}$. The material parameters are taken from[@acs_nanolett] and [@du_spontaneous_2022] respectively. The color bar represents normalized spin vectors in the z direction. Note that the spins of $\textrm{MnBr_2}$ appear purple because there are "red-blue" spin pairs for the vast majority.

![Figure 3](figures/Figure_4.png)
Fig 3: Lifetime of a skyrmion in $\textrm{MnSTe}$, from its creation to annihilation. The graph denotes the average energy per atom. As we approach the global minima, the entire field becomes aligned to the magnetic field as expected. Total time: $30s$.

# State of Field

THe state of micromagnetics have been moderately studied since the advent of computers with the power to compute. To this effect, some of the notable existing software would be [VAMPIRE](https://vampire.york.ac.uk/) and [SPIRIT](https://spirit-code.github.io/). While SPIRIT uses an LLG based approach to calculate the electron spin vectors, our code uses a Monte Carlo based approach, which aids in extended sampling of the phase space. VAMPIRE uses a Monte Carlo based approach but is primarily a CPU(C++ MPI) based code which is outdated by today's standards due to the advent and mass usage of the GPU architecture. CUDA-METRO was designed and written to achieve a modular GPU-based framework which would work well on modern hardware and use a more efficient approach the exploring the phase space of micromagnetics. 
