...
Fig 2: Presence of skyrmions in \ce{MnBr_2} and \ce{CrInSe_3}. The material parameters are taken from[@acs_nanolett] and [@du_spontaneous_2022] respectively. The color bar represents normalized spin vectors in the z direction. Note that the spins of \ce{MnBr_2} appear purple because there are "red-blue" spin pairs for the vast majority.

![Figure 3](figures/Figure_4.png)
Fig 3: Lifetime of a skyrmion in $\ce{MnSTe}$, from its creation to annihilation. The graph denotes the average energy per atom. As we approach the global minima, the entire field becomes aligned to the magnetic field as expected. Total time: $30s$.
