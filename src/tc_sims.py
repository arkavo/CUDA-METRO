...
for t in test_mc0.T:
    ...
    m, x = test_mc0.run_mc_tc_66612(t)
    M = np.append(M, m)
    X = np.append(X, x)
np.save(f"{test_mc0.save_direcotry}/BLM_{test_mc0.size}_{test_mc0.Blocks}", M)
np.save(f"{test_mc0.save_direcotry}/BLX_{test_mc0.size}_{test_mc0.Blocks}", X)
