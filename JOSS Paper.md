...
It was thought that long range order in materials of less than 3 dimensions would not exist, however, the material $CrI_3$ was shown to show ferromagnetism as a 2D material. Consequently, further studies have shown the existence of long range order in a lot more materials. In our first experiment, we show the phase transition of $CrI_3$ from ferromagnetic to paramagnetic with increasing temperature. Temperature causes the atomic spins to not be locked in to a ferromagnetic position but rather give them enough energy to orient themselves in any direction, the average direction of all the spins would then of course give a null vector, which gives rise to paramagnetism.
In Fig 1, we reproduce magnetic phase transition from [@augustin_npj] using ```CUDA-METRO```. We can see the effect of excess parallelization in $CrI_3$ in Fig 1 with respect to the reference. As mentioned before, we can obtain very accurate results with respect to SSU methods by fixing the parallelization to be $10\%$ of $N^2$.
